...
int check_format_32(Elf32_Ehdr *elf, char *path)
{
...
void display_as_ascii_32(int value, int *list, unsigned char *str, char *t)
{
...
void display_section_32(char *strtab, Elf32_Ehdr *elf, Elf32_Shdr *shdr, int a)
{
    ...
    while (tmp < (int)shdr[a].sh_size)
    {
        ...
        if (tmp % (int)16 == 0 || tmp >= (int)shdr[a].sh_size)
            display_as_ascii_32(tmp, &list, str, (char *)str);
    }
}

void next_display_32(Elf32_Ehdr *elf, Elf32_Shdr *shdr, char *strtab)
{
    ...
    for (int a = 0; a < elf->e_shnum; a++) {
        if (shdr[a].sh_size && strcmp(&strtab[shdr[a].sh_name], ".strtab") &&
            ...
            display_section_32(strtab, elf, shdr, a);
    }
}
...
void display_objdump_32(void *buf, char *filename)
{
    ...
    if (check_format_32(elf, filename) == 84)
        return;
    shdr = (Elf32_Shdr *)(buf + elf->e_shoff);
    ...
    next_display_32(elf, shdr, strtab);
}